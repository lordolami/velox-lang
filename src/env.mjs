...
export function validateEnv(schema = {}, env = process.env) {
  ...
  for (const [key, ruleRaw] of Object.entries(schema || {})) {
    ...
    if (type === "int") {
      ...
      if (!Number.isInteger(n)) errors.push(`${key} must be int`);
      else out[key] = n;
      continue;
    }
    if (type === "float" || type === "number") {
      const n = Number(value);
      if (!Number.isFinite(n)) errors.push(`${key} must be number`);
      else out[key] = n;
      continue;
    }
...
export async function validateAppEnv({ root = process.cwd() } = {}) {
  ...
  const mod = await import(`${new URL(`file://${schemaPath.replace(/\\/g, "/")}`).href}?t=${Date.now()}`);
